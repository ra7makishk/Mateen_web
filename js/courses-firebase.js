...
function renderModalMats() {
  ...
  Object.entries(modalMap).forEach(([subj, { id: modalId }]) => {
    // ── أزرار تعديل/حذف المادة الرئيسية (للأدمن والمعلمة) ──
    const adminActionsEl = document.getElementById('modal-admin-actions-' + modalId);
    if (adminActionsEl) {
      const canEditSubj = isAdmin() || (currentUserRole === 'teacher' && currentUserSubjects.includes(subj));
      if (canEditSubj) {
...
function subjectCardHTML(s) {
  const canEditSubj = isAdmin() || (currentUserRole === 'teacher' && currentUserSubjects.includes(s.name));
  const adminActions = canEditSubj ? `
    ...
    </div>` : ''; { initializeApp, getApps, getApp }
  from "https://www.gstatic.com/firebasejs/12.13.0/firebase-app.js";
import { getAuth, onAuthStateChanged }
  from "https://www.gstatic.com/firebasejs/12.13.0/firebase-auth.js";
import { getFirestore, collection, query, orderBy, onSnapshot, addDoc, getDoc, doc, updateDoc, deleteDoc, arrayUnion, getDocs, setDoc }
    ...
    <div class="modal-box">
      ...
      <div class="modal-content">
        ...
        <div class="modal-actions">
          ${(isAdmin() || (currentUserRole === 'teacher' && currentUserSubjects.includes(s.name))) ? `
          ...
          ${isAdmin() ? `<button onclick="document.getElementById('dynModal-${id}').remove();confirmDeleteSubject('${id}','${s.name.replace(/'/g, "\'")}')"
            ...
            <i class="ti ti-trash"></i> حذف
          </button>` : ''}` : ''}t { initializeApp, getApps, getApp }
  from "https://www.gstatic.com/firebasejs/12.13.0/firebase-app.js";
import { getAuth, onAuthStateChanged }
  from "https://www.gstatic.com/firebasejs/12.13.0/firebase-auth.js";
import { getFirestore, collection, query, orderBy, onSnapshot, addDoc, getDoc, doc, updateDoc, deleteDoc, arrayUnion, getDocs, setDoc }
