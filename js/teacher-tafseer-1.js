...
const TEACHER_ID = "tafseer";
const SUBJECT_AR = "التفسير";
...
async function loadTeacherName() {
  try {
    ...
    if (!snap.empty) {
      const name = snap.docs[0].data().name || 'معلمة التفسير';
...
onAuthStateChanged(auth, async user => {
  ...
  if (role === 'teacher' && snap.data().subject === 'tafseer') {
...
function loadInbox() {
  const q = query(
    collection(db, 'teachers', 'tafseer', 'messages'),
    orderBy('sentAt', 'desc')
  );
  onSnapshot(q, snap => {
...
window.openMsg = async (id, name, topic, body, phone, date) => {
  await updateDoc(doc(db, 'teachers', 'tafseer', 'messages', id), { read: true });
