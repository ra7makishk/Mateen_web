...
onAuthStateChanged(auth, async user => {
  ...
  if (role === 'teacher' && subject !== 'quran2') {
    window.location.href = 'home.html'; return;
  }
  loadTeacherName();
  // لو المعلمة نفسها — أظهر الـ inbox
  if (role === 'teacher' && snap.data().subject === 'quran2') {
...
function loadInbox() {
  const q = query(
    collection(db, 'teachers', 'quran2', 'messages'),
    orderBy('sentAt', 'desc')
  );
  onSnapshot(q, snap => {
...
window.openMsg = async (id, name, topic, body, phone, date) => {
  await updateDoc(doc(db, 'teachers', 'quran2', 'messages', id), { read: true });
