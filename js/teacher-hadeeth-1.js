...
const TEACHER_ID = "hadeeth";
const SUBJECT_AR = "الحديث الشريف";
...
async function loadTeacherName() {
  try {
    ...
    if (!snap.empty) {
      const name = snap.docs[0].data().name || 'معلمة الحديث';
...
onAuthStateChanged(auth, async user => {
  ...
  if (role === 'teacher' && subject !== 'hadeeth') {
    window.location.href = 'home.html'; return;
  }
  loadTeacherName();
  // لو المعلمة نفسها — أظهر الـ inbox
  if (role === 'teacher' && snap.data().subject === 'hadeeth') {
...
function loadInbox() {
  const q = query(
    collection(db, 'teachers', 'hadeeth', 'messages'),
    orderBy('sentAt', 'desc')
  );
  onSnapshot(q, snap => {
...
window.openMsg = async (id, name, topic, body, phone, date) => {
  await updateDoc(doc(db, 'teachers', 'hadeeth', 'messages', id), { read: true });
