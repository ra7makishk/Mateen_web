...
onAuthStateChanged(auth, async user => {
  ...
  if (role === 'teacher' && subject !== 'quran1') {
    window.location.href = 'home.html'; return;
  }
  loadTeacherName();
  // لو المعلمة نفسها — أظهر الـ inbox
  if (role === 'teacher' && snap.data().subject === 'quran1') {
...
function loadInbox() {
  const q = query(
    collection(db, 'teachers', 'quran1', 'messages'),
    orderBy('sentAt', 'desc')
  );
  onSnapshot(q, snap => {
...
window.openMsg = async (id, name, topic, body, phone, date) => {
  await updateDoc(doc(db, 'teachers', 'quran1', 'messages', id), { read: true });
